...
export default function BouclesOreillesPage() {
  const [bouclesOreilles, setBouclesOreilles] = useState([]);
  const [loading, setLoading] = useState(true);
  const { addToCart } = useCart();

  useEffect(() => {
    async function fetchBouclesOreilles() {
      setLoading(true);
      try {
        const response = await fetch('/api/products?category=bouclesOreilles');
        if (!response.ok) {
          throw new Error('Erreur lors de la récupération des boucles d\'oreilles');
        }
        const data = await response.json();
        setBouclesOreilles(data);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-center mb-8">Boucles d'Oreilles</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {bouclesOreilles.length > 0 ? (
              bouclesOreilles.map(product => (
              ...
              <div className="col-span-full text-center py-10">
                <p className="text-gray-500">Aucune boucle d'oreille disponible pour le moment.</p>
              </div>
            )}
          </div>
        )}
