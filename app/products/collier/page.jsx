...
export default function CollierPage() {
  const [collier, setCollier] = useState([]);
  const [loading, setLoading] = useState(true);
  const { addToCart } = useCart();

  useEffect(() => {
    async function fetchCollier() {
      setLoading(true);
      try {
        const response = await fetch('/api/products?category=colliers');
        if (!response.ok) {
          throw new Error('Erreur lors de la récupération des colliers');
        }
  ...
  return (
    <div className="bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-center mb-8">Colliers</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {collier.length > 0 ? (
              ...
              <div className="col-span-full text-center py-10">
                <p className="text-gray-500">Aucun collier disponible pour le moment.</p>
              </div>
            )}
          </div>
        )}
